...
class RickAdapter(val dataRick: List<ResultsItem>): RecyclerView.Adapter<RickAdapter.MyViewHolder>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_karakter,parent,false)
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.statusRick.text = dataRick?.get(position)?.name
        holder.speciesRick.text = dataRick?.get(position)?.name
